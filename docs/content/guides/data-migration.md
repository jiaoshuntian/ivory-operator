...
To help explain how these fields are used, we will consider a `pgcluster` from IVYO v4, `oldhippo`. We will assume that the `pgcluster` has been deleted and only the PVCs have been left in place.
...
With the above in mind, we need to reference the three PVCs we wish to migrate in the `dataSource.volumes` portion of the PostgresCluster spec. Additionally, to accommodate the IVYO v5 file structure, we must also reference the pgData and pgBackRest repo directories. Note that the pg_wal directory does not need to be moved when migrating from v4 to v5!
...
Lastly, it is very important that the IvorySQL version and storage configuration in your PostgresCluster match *exactly* the existing volumes being used.

If the volumes were used with IvorySQL 13, the `spec.postgresVersion` value should be `13` and the associated `spec.image` value should refer to a IvorySQL 13 image.
...
### Removing IVYO v4 labels

When migrating data volumes from v4 to v5, IVYO relabels all volumes for IVYO v5, but **will not remove existing IVYO v4 labels**. This results in PVCs that are labeled for both IVYO v4 and v5, which can lead to unintended behavior.

To avoid that, you must manually remove the `pg-cluster` and `vendor` labels, which you can do with a `kubectl` command. For instance, given a cluster named `hippo` with a dedicated pgBackRest repo, the PVC will be `hippo-pgbr-repo`, and the IVYO v4 labels can be removed with the below command:
...
When migrating from IVYO v4, this will require the user to manually set the group value of the pgBackRest repo directory, and all subdirectories, to `26` to match the `postgres` group used in IVYO v5. Please see [here](https://github.com/kubernetes/examples/issues/260) for more information.
...
- When migrating from IVYO v4 volumes, it is recommended to use the most recently available version of IVYO v4.
