...
name_desc_with_todo=$(
  python3 -m yq -r \
    ...
    "${clusters_dir}/generated/ivory-operator.ivorysql.org_ivoryclusters.yaml"
)
...
python3 -m yq -y --arg old "${name_desc_with_todo}" --arg new "${name_desc_without_todo}" '
	...
	[{ op: "remove", path: "/work" }]
' \
	"${clusters_dir}/generated/ivory-operator.ivorysql.org_ivoryclusters.yaml" > "${clusters_dir}/todos.yaml"
...
python3 -m yq -y --arg old "${name_desc_with_todo}" --arg new "${name_desc_without_todo}" '
	...
	[{ op: "remove", path: "/work" }]
' \
	"${upgrades_dir}/generated/ivory-operator.ivorysql.org_ivyupgrades.yaml" > "${upgrades_dir}/todos.yaml"
