...
func (r *Reconciler) reconcileClusterConfigMap(
	...
	if err == nil {
		err = patroni.ClusterConfigMap(ctx, cluster, ivyHBAs, pgParameters,
			clusterConfigMap)
	}
	if err == nil {
...
// resolve to the IvorySQL primary instance.
func (r *Reconciler) generateClusterPrimaryService(
	cluster *v1beta1.IvoryCluster, leader *corev1.Service,
	...
	service.Spec.Ports = []corev1.ServicePort{{
		Name:       naming.PortIvorySQL,
		Port:       *cluster.Spec.Port,
		Protocol:   corev1.ProtocolTCP,
		TargetPort: intstr.FromString(naming.PortIvorySQL),
	}}
...
// to the IvorySQL primary instance.
func (r *Reconciler) reconcileClusterPrimaryService(
	ctx context.Context, cluster *v1beta1.IvoryCluster, leader *corev1.Service,
) (*corev1.Service, error) {
...
// generateClusterReplicaService returns a v1.Service that exposes IvorySQL
// replica instances.
func (r *Reconciler) generateClusterReplicaService(
	cluster *v1beta1.IvoryCluster) (*corev1.Service, error,
) {
	...
	// The TargetPort must be the name (not the number) of the IvorySQL
...
// reconcileClusterReplicaService writes the Service that exposes IvorySQL
// replica instances.
...
// reconcileDataSource is responsible for reconciling the data source for a IvorySQL cluster.
// This involves ensuring the IvorySQL data directory for the cluster is properly populated
// prior to bootstrapping the cluster, specifically according to any data source configured in the
// IvoryCluster spec.
// TODO(benjaminjb): Right now the spec will accept a dataSource with both a IvoryCluster and
// a PGBackRest section, but the code will only honor the IvoryCluster in that case; this would
// be better handled with a webhook to reject a spec with both `dataSource.ivoryCluster` and
...
func (r *Reconciler) reconcileDataSource(ctx context.Context,
	...
	// determine if the user wants to initialize the IVY data directory
	ivoryDataInitRequested := cluster.Spec.DataSource != nil &&
		(cluster.Spec.DataSource.IvoryCluster != nil ||
	...
	// Set the proper data source for the restore based on whether we're initializing the IVY
	// data directory (e.g. for a new IvorySQL cluster), or restoring an existing cluster
	// in place (and therefore recreating the data directory).  If the user hasn't requested
	// IVY data initialization or an in-place restore, then simply return.
	var dataSource *v1beta1.IvoryClusterDataSource
	var cloudDataSource *v1beta1.PGBackRestDataSource
	switch {
	case restoreInPlaceRequested:
		dataSource = cluster.Spec.Backups.PGBackRest.Restore.IvoryClusterDataSource
	case ivoryDataInitRequested:
		// there is no restore annotation when initializing a new cluster, so we create a
		// restore ID for bootstrap
		restoreID = "~ivyo-bootstrap-" + cluster.GetName()
		dataSource = cluster.Spec.DataSource.IvoryCluster
		if dataSource == nil {
	...
	// check the cluster's conditions to determine if the IVY data for the cluster has been
	// initialized
	dataSourceCondition := meta.FindStatusCondition(cluster.Status.Conditions,
		ConditionIvoryDataInitialized)
	ivoryDataInitialized := dataSourceCondition != nil &&
	...
	if ivoryDataInitialized {
		return false, nil
	}

	// proceed with initializing the IVY data directory if not already initialized
	switch {
	case dataSource != nil:
		if err := r.reconcileIvoryClusterDataSource(ctx, cluster, dataSource,
	...
	// return early until the IVY data directory is initialized
	return true, nil
}
