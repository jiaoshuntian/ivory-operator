...
func (r *Reconciler) Reconcile(
	...
	// get the ivorycluster from the cache
	...
	if err := r.Client.Get(ctx, request.NamespacedName, cluster); err != nil {
		...
		if err = client.IgnoreNotFound(err); err != nil {
			log.Error(err, "unable to fetch IvoryCluster")
	...
	// IvoryCluster may be deleted before the IvoryCluster is deleted.
	...
	// Define the function for the updating the IvoryCluster status. Returns any error that
	// occurs while attempting to patch the status, while otherwise simply returning the
	// Result and error variables that are populated while reconciling the IvoryCluster.
	...
	if cluster.Spec.Paused != nil && *cluster.Spec.Paused {
		meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
			Type:    v1beta1.IvoryClusterProgressing,
		...
		meta.RemoveStatusCondition(&cluster.Status.Conditions, v1beta1.IvoryClusterProgressing)
	}

	ivyHBAs := ivory.NewHBAs()
	pgmonitor.IvorySQLHBAs(cluster, &ivyHBAs)
	pgbouncer.IvorySQL(cluster, &ivyHBAs)

	pgParameters := ivory.NewParameters()
	pgaudit.IvorySQLParameters(&pgParameters)
	pgbackrest.IvorySQL(cluster, &pgParameters)
	pgmonitor.IvorySQLParameters(cluster, &pgParameters)

	// Set huge_pages = try if a hugepages resource limit > 0, otherwise set "off"
	ivory.SetHugePages(cluster, &pgParameters)
	...
	// First handle reconciling any data source configured for the IvoryCluster.  This includes
	...
	if err == nil {
		// Since the IvorySQL data source needs to be populated prior to bootstrapping the
	...
	if err == nil {
		clusterConfigMap, err = r.reconcileClusterConfigMap(ctx, cluster, ivyHBAs, pgParameters)
	}
	if err == nil {
	...
	if err == nil {
		err = r.reconcilePatroniDynamicConfiguration(ctx, cluster, instances, ivyHBAs, pgParameters)
	}
	if err == nil {
	...
	if err == nil {
		err = r.reconcileIvoryDatabases(ctx, cluster, instances)
	}
	if err == nil {
		err = r.reconcileIvoryUsers(ctx, cluster, instances)
	}
...
func (r *Reconciler) deleteControlled(
	ctx context.Context, cluster *v1beta1.IvoryCluster, object client.Object,
) error {
...
func (r *Reconciler) setControllerReference(
	owner *v1beta1.IvoryCluster, controlled client.Object,
) error {
...
func (r *Reconciler) SetupWithManager(mgr manager.Manager) error {
	...
	if s := os.Getenv("IVYO_WORKERS"); s != "" {
		if i, err := strconv.Atoi(s); err == nil && i > 0 {
			opts.MaxConcurrentReconciles = i
		} else {
			mgr.GetLogger().Error(err, "IVYO_WORKERS must be a positive number")
		}
	}
