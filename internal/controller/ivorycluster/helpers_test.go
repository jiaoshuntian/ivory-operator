...
var (
	...
	IvoryHAImage       = "registry.developers.highgo.com/highgo/highgo-ivory:ubi8-13.6-1"
	IvoryBackRestImage = "registry.developers.highgo.com/highgo/highgo-pgbackrest:ubi8-2.38-0"
	IvoryBouncerImage  = "registry.developers.highgo.com/highgo/highgo-pgbouncer:ubi8-1.16-2"
)
...
func init() {
	setting := os.Getenv("IVYO_TEST_TIMEOUT_SCALE")
	factor, _ := strconv.ParseFloat(setting, 64)

	if setting != "" {
		if factor <= 0 {
			panic("IVYO_TEST_TIMEOUT_SCALE must be a fractional number greater than zero")
		}
...
func setupKubernetes(t testing.TB) (*envtest.Environment, client.Client) {
	...
	t.Cleanup(func() {
		...
		if t.Failed() {
			if cc, err := client.New(kubernetes.env.Config, client.Options{}); err == nil {
				...
				_ = cc.List(context.Background(), &namespaces, client.HasLabels{"ivory-operator-test"})

				type shaped map[string]corev1.NamespaceStatus
				result := make([]shaped, len(namespaces.Items))

				for i, ns := range namespaces.Items {
					result[i] = shaped{ns.Labels["ivory-operator-test"]: ns.Status}
				}
	...
	scheme, err := runtime.CreateIvoryOperatorScheme()
	assert.NilError(t, err)
...
func setupNamespace(t testing.TB, cc client.Client) *corev1.Namespace {
	t.Helper()
	ns := &corev1.Namespace{}
	ns.GenerateName = "ivory-operator-test-"
...
func testCluster() *v1beta1.IvoryCluster {
	...
	cluster := v1beta1.IvoryCluster{
		...
		Spec: v1beta1.IvoryClusterSpec{
			...
			Proxy: &v1beta1.IvoryProxySpec{
				PGBouncer: &v1beta1.PGBouncerPodSpec{
					Image: IvoryBouncerImage,
				},
			},
		},
	}
