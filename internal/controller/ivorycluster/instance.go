...
// IsWritable returns whether or not a IvorySQL connection could write to its
// database.
...
// observedInstances represents all the IvorySQL instances of a single IvoryCluster.
...
func newObservedInstances(
	...
	sets := make(map[string]*v1beta1.IvoryInstanceSetSpec)
...
func (r *Reconciler) observeInstances(
	...
	for _, name := range observed.setNames.List() {
		status := v1beta1.IvoryInstanceSetStatus{Name: name}
...
// unclean shutdowns of IvorySQL. It returns (nil, nil) when finished.
...
func findAvailableInstanceNames(set v1beta1.IvoryInstanceSetSpec,
	...
	// any available IVYDATA volumes for the instance set that have no corresponding WAL
	...
	if set.WALVolumeClaimSpec != nil {
		...
		for _, setVol := range setVolumes {
			...
			for _, pvc := range clusterVolumes {
				...
				pvcRole := pvc.GetLabels()[naming.LabelRole]
				if pvcRole == naming.RoleIvoryWAL && pvcSet == set.Name &&
...
func (r *Reconciler) rolloutInstance(
	...
	// choose the best candidate and demote the primary. It stops IvorySQL
	...
	// - https://www.ivorysql.org/docs/current/sql-checkpoint.html
	//
	...
	if primary {
		...
		checkpoint := func(ctx context.Context) (time.Duration, error) {
			ctx, span := r.Tracer.Start(ctx, "ivorysql-checkpoint")
			defer span.End()

			start := time.Now()
			stdout, stderr, err := ivory.Executor(exec).
	...
	// IvorySQL without taking a checkpoint.
...
func (r *Reconciler) scaleUpInstances(
	...
	cluster *v1beta1.IvoryCluster,
	observed *observedInstances,
	set *v1beta1.IvoryInstanceSetSpec,
	clusterConfigMap *corev1.ConfigMap,
	clusterReplicationSecret *corev1.Secret,
	rootCA *pki.RootCertificateAuthority,
	clusterPodService *corev1.Service,
...
func (r *Reconciler) reconcileInstance(
	...
	*instance = appsv1.StatefulSet{}
	...
	var (
		...
		ivoryDataVolume      *corev1.PersistentVolumeClaim
		ivoryWALVolume       *corev1.PersistentVolumeClaim
	...
	if err == nil {
		ivoryDataVolume, err = r.reconcileIvoryDataVolume(ctx, cluster, spec, instance, clusterVolumes)
	}
	if err == nil {
		ivoryWALVolume, err = r.reconcileIvoryWALVolume(ctx, cluster, spec, instance, observed, clusterVolumes)
	}
	if err == nil {
	...
	if err == nil {
		ivory.InstancePod(
			...
			ivoryDataVolume, ivoryWALVolume, tablespaceVolumes,
	...
	if err == nil {
		addNSSWrapper(
			config.IvoryContainerImage(cluster),
	...
	// mount shared memory to the Ivory instance
	if err == nil {
...
func generateInstanceStatefulSetIntent(_ context.Context,
	...
	// method considers Pods across the entire IvoryCluster and deletes
	...
	if cluster.Spec.DisableDefaultPodScheduling == nil ||
		...
		sts.Spec.Template.Spec.TopologySpreadConstraints = append(
			...
			defaultTopologySpreadConstraints(
				naming.ClusterDataForIvoryAndPGBackRest(cluster.Name),
			)...)
	}
	...
	sts.Spec.Template.Spec.SecurityContext = ivory.PodSecurityContext(cluster)
...
// to the PodSpec.
func addPGBackRestToInstancePodSpec(cluster *v1beta1.IvoryCluster,
...
func (r *Reconciler) reconcileInstanceConfigMap(
	ctx context.Context, cluster *v1beta1.IvoryCluster, spec *v1beta1.IvoryInstanceSetSpec,
	instance *appsv1.StatefulSet,
...
func (r *Reconciler) reconcileInstanceCertificates(
	ctx context.Context, cluster *v1beta1.IvoryCluster,
	spec *v1beta1.IvoryInstanceSetSpec, instance *appsv1.StatefulSet,
