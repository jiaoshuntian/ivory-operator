...
func TestNewObservedInstances(t *testing.T) {
	...
	t.Run("PodMissingOthers", func(t *testing.T) {
		...
		assert.Assert(t, instance.Spec == nil)   // No matching IvoryInstanceSetSpec
	...
	t.Run("Matching", func(t *testing.T) {
		cluster := new(v1beta1.IvoryCluster)
		cluster.Spec.InstanceSets = []v1beta1.IvoryInstanceSetSpec{{Name: "00"}}
		...
		assert.Equal(t, len(instance.Pods), 1)   // The Pod
		assert.Assert(t, instance.Runner != nil) // The StatefulSet
		assert.Assert(t, instance.Spec != nil)   // The IvoryInstanceSetSpec
...
`))
		...
		t.Run(test.name, func(t *testing.T) {
			...
			spec := test.ip.spec
			if spec == nil {
				spec = new(v1beta1.IvoryInstanceSetSpec)
				spec.Default(0)
			}
...
func TestFindAvailableInstanceNames(t *testing.T) {
	...
	testCases := []struct {
		...
		set: v1beta1.IvoryInstanceSetSpec{Name: "instance1"},
		fakeObservedInstances: newObservedInstances(
			&v1beta1.IvoryCluster{Spec: v1beta1.IvoryClusterSpec{
				InstanceSets: []v1beta1.IvoryInstanceSetSpec{{}},
			}},
			[]appsv1.StatefulSet{{}},
			[]corev1.Pod{},
		...
		fakeObservedInstances: newObservedInstances(
			&v1beta1.IvoryCluster{Spec: v1beta1.IvoryClusterSpec{
				InstanceSets: []v1beta1.IvoryInstanceSetSpec{{Name: "instance1"}},
			}},
...
func TestCleanupDisruptionBudgets(t *testing.T) {
	...
	t.Run("pdbs found", func(t *testing.T) {
		...
		t.Run("cleanup leftover pdb", func(t *testing.T) {
			leftoverPDB := generatePDB(t, cluster, &v1beta1.IvoryInstanceSetSpec{
				Name:     "old-instance",
				Replicas: initialize.Int32(1),
			}, initialize.IntOrStringInt32(1))
