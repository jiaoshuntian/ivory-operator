...
func (r *Reconciler) handlePatroniRestarts(
	...
	// Some IvorySQL settings must be changed on the primary before any
	...
	//
	// NOTE: In Patroni v2.1.1, regardless of the IvorySQL parameter, the
	...
	// When we offer per-instance IvorySQL configuration, we may need to revisit
	// how we decide when to restart.
	// - https://www.ivorysql.org/docs/current/runtime-config-replication.html
...
func (r *Reconciler) reconcilePatroniDynamicConfiguration(
	...
	configuration = patroni.DynamicConfiguration(cluster, configuration, ivyHBAs, pgParameters)
...
func (r *Reconciler) reconcileReplicationSecret(
	...
	// if a custom ivorycluster secret is provided, just return it
	...
	commonName := ivory.ReplicationUser
...
// replicationCertSecretProjection returns a secret projection of the ivorycluster's
