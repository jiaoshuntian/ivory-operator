...
func TestPatroniReplicationSecret(t *testing.T) {
	...
	rootCA, err := r.reconcileRootCertificate(ctx, ivoryCluster)
	assert.NilError(t, err)

	t.Run("reconcile", func(t *testing.T) {
		_, err = r.reconcileReplicationSecret(ctx, ivoryCluster, rootCA)
		assert.NilError(t, err)
	})

	t.Run("validate", func(t *testing.T) {

		patroniReplicationSecret := &corev1.Secret{ObjectMeta: naming.ReplicationClientCertSecret(ivoryCluster)}
	...
	t.Run("check replication certificate secret projection", func(t *testing.T) {
		...
		testSecretProjection := &corev1.SecretProjection{
			LocalObjectReference: corev1.LocalObjectReference{
				Name: naming.ReplicationClientCertSecret(ivoryCluster).Name,
			},
		...
		testReplicationSecret, err := r.reconcileReplicationSecret(ctx, ivoryCluster, rootCA)
		assert.NilError(t, err)
...
func TestReconcilePatroniStatus(t *testing.T) {
	...
	createResources := func(index, readyReplicas int,
		writeAnnotation bool) (*v1beta1.IvoryCluster, *observedInstances) {
		...
		return ivoryCluster, observedInstances
	}
	...
	for i, tc := range testsCases {
		t.Run(fmt.Sprintf("%+v", tc), func(t *testing.T) {
			ivoryCluster, observedInstances := createResources(i, tc.readyReplicas,
				tc.writeAnnotation)
			result, err := r.reconcilePatroniStatus(ctx, ivoryCluster, observedInstances)
...
func TestReconcilePatroniSwitchover(t *testing.T) {
	...
	t.Run("early validation", func(t *testing.T) {
		for _, test := range []struct {
			...
			soType  string
			target  string
			check   func(*testing.T, error, *v1beta1.IvoryCluster)
		}{
			{
				desc:    "Switchover not enabled",
				enabled: false,
				check: func(t *testing.T, err error, cluster *v1beta1.IvoryCluster) {
					assert.NilError(t, err)
			...
			t.Run(test.desc, func(t *testing.T) {
				...
				if test.status != "" {
					cluster.Status = v1beta1.IvoryClusterStatus{
