...
import (
	...
	"github.com/highgo/ivory-operator/internal/patroni"
...
// proper configuration according to the provided IvoryCluster custom resource.  This is done by
// applying the IvoryCluster controller's fully specified intent for the repository host
...
// strategy.
func (r *Reconciler) applyRepoHostIntent(ctx context.Context, ivoryCluster *v1beta1.IvoryCluster,
	repoHostName string, repoResources *RepoResources,
	observedInstances *observedInstances) (*appsv1.StatefulSet, error) {

	repo, err := r.generateRepoHostIntent(ivoryCluster, repoHostName, repoResources,
		observedInstances)
	if err != nil {
		return nil, err
	}
...
func (r *Reconciler) applyRepoVolumeIntent(ctx context.Context,
	...
	if err := r.apply(ctx, repo); err != nil {
		return nil, r.handlePersistentVolumeClaimError(ivoryCluster,
...
// IvoryCluster controller during reconciliation.  Any items returned are verified to be owned
// by the IvoryCluster controller and still applicable per the current IvoryCluster spec.
...
func (r *Reconciler) getPGBackRestResources(ctx context.Context,
	ivoryCluster *v1beta1.IvoryCluster) (*RepoResources, error) {
	...
	selector := naming.PGBackRestSelector(ivoryCluster.GetName())
	for _, gvk := range gvks {
		uList := &unstructured.UnstructuredList{}
		uList.SetGroupVersionKind(gvk)
		if err := r.Client.List(ctx, uList,
			client.InNamespace(ivoryCluster.GetNamespace()),
		...
		owned, err := r.cleanupRepoResources(ctx, ivoryCluster, uList.Items)
		if err != nil {
			return nil, errors.WithStack(err)
		}
		uList.Items = owned
		...
		if gvk.Kind == "JobList" {
			r.setScheduledJobStatus(ctx, ivoryCluster, uList.Items)
		}
...
// are no longer associated with any repository configured within the IvoryCluster spec, or any
...
func (r *Reconciler) cleanupRepoResources(ctx context.Context,
	...
	for i, owned := range ownedResources {
		...
		// helper to determine if a label is present in the IvoryCluster
		...
		// determines whether or not it should be deleted according to the current IvoryCluster
		// spec
		switch {
		...
		case hasLabel(naming.LabelPGBackRestDedicated):
			...
			if owned.GetName() != naming.PGBackRestSSHConfig(ivoryCluster).Name &&
				owned.GetName() != naming.PGBackRestSSHSecret(ivoryCluster).Name {
				// If a dedicated repo host resource and a dedicated repo host is enabled, then
				// add to the slice and do not delete.
				if pgbackrest.DedicatedRepoHostEnabled(ivoryCluster) {
					ownedNoDelete = append(ownedNoDelete, owned)
					delete = false
				}
			}
		...
		case hasLabel(naming.LabelPGBackRestRestore):
			...
			if !patroni.ClusterBootstrapped(ivoryCluster) {
				ownedNoDelete = append(ownedNoDelete, owned)
				delete = false
			}
		}
...
// defined by the ivorycluster CRD, otherwise it returns false.
...
func (r *Reconciler) setScheduledJobStatus(ctx context.Context,
	...
	if ivoryCluster.Status.PGBackRest == nil {
		ivoryCluster.Status.PGBackRest = &v1beta1.PGBackRestStatus{}
	}
	ivoryCluster.Status.PGBackRest.ScheduledBackups = scheduledStatus
}

// generateRepoHostIntent creates and populates StatefulSet with the IvoryCluster's full intent
// as needed to create and reconcile a pgBackRest dedicated repository host within the kubernetes
// cluster.
func (r *Reconciler) generateRepoHostIntent(ivoryCluster *v1beta1.IvoryCluster,
	...
	labels := naming.Merge(
		...
		naming.PGBackRestDedicatedLabels(ivoryCluster.GetName()),
		map[string]string{
	...
	repo := &appsv1.StatefulSet{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:        repoHostName,
			Namespace:   ivoryCluster.GetNamespace(),
			Labels:      labels,
			Annotations: annotations,
		},
		Spec: appsv1.StatefulSetSpec{
			Selector: &metav1.LabelSelector{
				MatchLabels: naming.PGBackRestDedicatedLabels(ivoryCluster.GetName()),
			},
			ServiceName: naming.ClusterPodService(ivoryCluster).Name,
	...
	if repoHost := ivoryCluster.Spec.Backups.PGBackRest.RepoHost; repoHost != nil {
	...
	if ivoryCluster.Spec.DisableDefaultPodScheduling == nil ||
		(ivoryCluster.Spec.DisableDefaultPodScheduling != nil &&
			!*ivoryCluster.Spec.DisableDefaultPodScheduling) {
		repo.Spec.Template.Spec.TopologySpreadConstraints = append(
			...
			defaultTopologySpreadConstraints(
				naming.ClusterDataForIvoryAndPGBackRest(ivoryCluster.Name),
			)...)
	}
	...
	repo.Spec.Template.Spec.ImagePullSecrets = ivoryCluster.Spec.ImagePullSecrets

	// determine if any IVY Pods still exist
	...
	if ivoryCluster.Spec.Shutdown != nil && *ivoryCluster.Spec.Shutdown &&
		!instancePodExists {
	...
	repo.Spec.Template.Spec.SecurityContext = ivory.PodSecurityContext(ivoryCluster)

	pgbackrest.AddServerToRepoPod(ivoryCluster, &repo.Spec.Template.Spec)

	// add the init container to make the pgBackRest repo volume log directory
	pgbackrest.MakePGBackrestLogDir(&repo.Spec.Template, ivoryCluster)

	// add pgBackRest repo volumes to pod
	if err := pgbackrest.AddRepoVolumesToPod(ivoryCluster, &repo.Spec.Template,
		getRepoPVCNames(ivoryCluster, repoResources.pvcs),
		naming.PGBackRestRepoContainerName); err != nil {
		return nil, errors.WithStack(err)
	}
	// add configs to pod
	pgbackrest.AddConfigToRepoPod(ivoryCluster, &repo.Spec.Template.Spec)

	// add nss_wrapper init container and add nss_wrapper env vars to the pgbackrest
	// container
	addNSSWrapper(
		config.PGBackRestContainerImage(ivoryCluster),
		ivoryCluster.Spec.ImagePullPolicy,
		&repo.Spec.Template)

	addTMPEmptyDir(&repo.Spec.Template)

	// set ownership references
	if err := controllerutil.SetControllerReference(ivoryCluster, repo,
...
func (r *Reconciler) generateRepoVolumeIntent(ivoryCluster *v1beta1.IvoryCluster,
	...
	labels := naming.Merge(
		ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestRepoVolumeLabels(ivoryCluster.GetName(), repoName),
	)

	// generate the default metadata
	meta := naming.PGBackRestRepoVolume(ivoryCluster, repoName)

	// but if there is an existing volume for this PVC, use it
	repoPVCNames := getRepoPVCNames(ivoryCluster, repoResources.pvcs)
	...
	// set ownership references
	if err := controllerutil.SetControllerReference(ivoryCluster, repoVol,
		r.Client.Scheme()); err != nil {
		return nil, err
	}
...
func generateBackupJobSpecIntent(ivoryCluster *v1beta1.IvoryCluster,
	...
	selector, containerName, err := getPGBackRestExecSelector(ivoryCluster, repo)
	if err != nil {
		return nil, errors.WithStack(err)
	}
	...
	container := corev1.Container{
		...
		Env: []corev1.EnvVar{
			...
			{Name: "NAMESPACE", Value: ivoryCluster.GetNamespace()},
			{Name: "SELECTOR", Value: selector.String()},
		},
		Image:           config.PGBackRestContainerImage(ivoryCluster),
		ImagePullPolicy: ivoryCluster.Spec.ImagePullPolicy,
		Name:            naming.PGBackRestRepoContainerName,
		SecurityContext: initialize.RestrictedSecurityContext(),
	}

	if ivoryCluster.Spec.Backups.PGBackRest.Jobs != nil {
		container.Resources = ivoryCluster.Spec.Backups.PGBackRest.Jobs.Resources
	}
	...
	if jobs := ivoryCluster.Spec.Backups.PGBackRest.Jobs; jobs != nil {
	...
	if ivoryCluster.Spec.Backups.PGBackRest.Jobs != nil {
		if ivoryCluster.Spec.Backups.PGBackRest.Jobs.PriorityClassName != nil {
			jobSpec.Template.Spec.PriorityClassName =
				*ivoryCluster.Spec.Backups.PGBackRest.Jobs.PriorityClassName
		}
		jobSpec.Template.Spec.Tolerations = ivoryCluster.Spec.Backups.PGBackRest.Jobs.Tolerations
		jobSpec.Template.Spec.Affinity = ivoryCluster.Spec.Backups.PGBackRest.Jobs.Affinity
	}
	...
	// of propagation to existing pods when the CRD is updated:
	// https://github.com/kubernetes/kubernetes/issues/88456
	jobSpec.Template.Spec.ImagePullSecrets = ivoryCluster.Spec.ImagePullSecrets

	// add pgBackRest configs to template
	if containerName == naming.PGBackRestRepoContainerName {
		pgbackrest.AddConfigToRepoPod(ivoryCluster, &jobSpec.Template.Spec)
	} else {
		pgbackrest.AddConfigToInstancePod(ivoryCluster, &jobSpec.Template.Spec)
	}
...
func (r *Reconciler) observeRestoreEnv(ctx context.Context,
	cluster *v1beta1.IvoryCluster) ([]corev1.Endpoints, *batchv1.Job, error) {
	...
	if restoreJob != nil {
		...
		if completed {
			meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
				...
				Type:               ConditionIvoryDataInitialized,
...
// prepareForRestore is responsible for reconciling an in place restore for the IvoryCluster.
...
// longer running, the "IvoryDataInitialized" condition is removed, which will cause the
// cluster to re-bootstrap using a restored data directory.
func (r *Reconciler) prepareForRestore(ctx context.Context,
	cluster *v1beta1.IvoryCluster, observed *observedInstances,
	...
	if len(currentEndpoints) == 0 {
		meta.RemoveStatusCondition(&cluster.Status.Conditions, ConditionIvoryDataInitialized)
		meta.SetStatusCondition(&cluster.Status.Conditions, metav1.Condition{
			ObservedGeneration: cluster.GetGeneration(),
			Type:               ConditionPGBackRestRestoreProgressing,
			Status:             metav1.ConditionTrue,
...
func (r *Reconciler) reconcileRestoreJob(ctx context.Context,
	...
	pgdata := ivory.DataDirectory(cluster)
	...
	// (b) our restore job assumes the `hot_standby: on` default, which is true of Ivory >= 10;
	// (c) pgBackRest passes the `--target-action` setting as `recovery-target-action`
	// in IvorySQL versions >=9.5 and as `pause_at_recovery_target` on earlier 9.x versions.
	...
	// - https://www.ivorysql.org/docs/14/runtime-config-wal.html#RUNTIME-CONFIG-WAL-RECOVERY-TARGET
	...
	for i, instanceSpec := range cluster.Spec.InstanceSets {
		if instanceSpec.Name == instanceSetName {
			opts = append(opts, "--link-map=pg_wal="+ivory.WALDirectory(cluster,
	...
	// create the volume resources required for the ivory data directory
	dataVolumeMount := ivory.DataVolumeMount()
	...
	if pgwalVolume != nil {
		walVolumeMount := ivory.WALVolumeMount()
	...
	for _, pgtablespaceVolume := range pgtablespaceVolumes {
		tablespaceVolumeMount := ivory.TablespaceVolumeMount(
...
func (r *Reconciler) generateRestoreJobIntent(cluster *v1beta1.IvoryCluster,
	configHash, instanceName string, cmd []string,
	volumeMounts []corev1.VolumeMount, volumes []corev1.Volume,
	dataSource *v1beta1.IvoryClusterDataSource, job *batchv1.Job) error {
		...
		Template: corev1.PodTemplateSpec{
			...
			Spec: corev1.PodSpec{
				Containers: []corev1.Container{{
					Command:         cmd,
					Image:           config.IvoryContainerImage(cluster),
	...
	job.Spec.Template.Spec.SecurityContext = ivory.PodSecurityContext(cluster)
...
// specific IvoryCluster (e.g. Deployments, ConfigMaps, Secrets, etc.).  This function will
...
func (r *Reconciler) reconcilePGBackRest(ctx context.Context,
	...
	repoResources, err := r.getPGBackRestResources(ctx, ivoryCluster)
	if err != nil {
	...
	dedicatedEnabled := pgbackrest.DedicatedRepoHostEnabled(ivoryCluster)
	if dedicatedEnabled {
		// reconcile the pgbackrest repository host
		repoHost, err = r.reconcileDedicatedRepoHost(ctx, ivoryCluster, repoResources, instances)
		if err != nil {
		...
		meta.RemoveStatusCondition(&ivoryCluster.Status.Conditions, ConditionRepoHostReady)
	}

	if err := r.reconcilePGBackRestSecret(ctx, ivoryCluster, repoHost, rootCA); err != nil {
	...
	configHashes, configHash, err := pgbackrest.CalculateConfigHashes(ivoryCluster)
	if err != nil {
	...
	replicaCreateRepo, err := r.reconcileRepos(ctx, ivoryCluster, configHashes, repoResources)
	if err != nil {
		log.Error(err, "unable to reconcile pgBackRest repo host")
		result = updateReconcileResult(result, reconcile.Result{Requeue: true})
		return result, nil
	...
	if err := r.reconcilePGBackRestConfig(ctx, ivoryCluster, repoHostName,
		configHash, naming.ClusterPodService(ivoryCluster).Name,
		ivoryCluster.GetNamespace(), instanceNames); err != nil {
	...
	sa, err := r.reconcilePGBackRestRBAC(ctx, ivoryCluster)
	if err != nil {
	...
	requeue := r.reconcileScheduledBackups(ctx, ivoryCluster, sa, repoResources.cronjobs)
	...
	if err := r.reconcileReplicaCreateBackup(ctx, ivoryCluster, instances,
	...
	// annotation
	if err := r.reconcileManualBackup(ctx, ivoryCluster, repoResources.manualBackupJobs,
...
// reconcileIvoryClusterDataSource is responsible for reconciling a IvoryCluster data source.
// This is specifically done by running a pgBackRest restore to populate a IvorySQL data volume
// for the IvoryCluster being reconciled using the backups of another IvoryCluster.
func (r *Reconciler) reconcileIvoryClusterDataSource(ctx context.Context,
	cluster *v1beta1.IvoryCluster, dataSource *v1beta1.IvoryClusterDataSource,
	...
	// below (e.g. the IVYDATA and/or WAL volumes).
	var instanceSet *v1beta1.IvoryInstanceSetSpec
	...
	// cluster, then we do not need to lookup a cluster and simply copy the current IvoryCluster.
	...
	// data directory for a brand new IvoryCluster using existing backups for that cluster).
	// If the source cluster is not the same as the current cluster, then look it up.
	sourceCluster := &v1beta1.IvoryCluster{}
	if sourceClusterName == cluster.GetName() && sourceClusterNamespace == cluster.GetNamespace() {
		...
		if err != nil || result != (reconcile.Result{}) {
			return fmt.Errorf("unable to reconcile pgBackRest as needed to initialize "+
				"IvorySQL data for the cluster: %w", err)
		}
	} else {
		if err := r.Client.Get(ctx,
			...
			if apierrors.IsNotFound(err) {
				r.Recorder.Eventf(cluster, corev1.EventTypeWarning, "InvalidDataSource",
					"IvoryCluster %q does not exist", sourceClusterName)
				return nil
			}
			return errors.WithStack(err)
		}
	...
	if !foundRepo {
		r.Recorder.Eventf(cluster, corev1.EventTypeWarning, "InvalidDataSource",
			"IvoryCluster %q does not have a repo named %q defined",
	...
	// Reconcile the IVYDATA and WAL volumes for the restore
	pgdata, err := r.reconcileIvoryDataVolume(ctx, cluster, instanceSet, fakeSTS, clusterVolumes)
	if err != nil {
		return errors.WithStack(err)
	}
	pgwal, err := r.reconcileIvoryWALVolume(ctx, cluster, instanceSet, fakeSTS, nil, clusterVolumes)
	if err != nil {
		return errors.WithStack(err)
	}
...
// reconcileCloudBasedDataSource is responsible for reconciling a cloud-based IvoryCluster
// data source, i.e., S3, etc.
func (r *Reconciler) reconcileCloudBasedDataSource(ctx context.Context,
	cluster *v1beta1.IvoryCluster, dataSource *v1beta1.PGBackRestDataSource,
	...
	// The `reconcileRestoreJob` was originally designed to take a IvoryClusterDataSource
	// and rather than reconfigure that func's signature, we translate the PGBackRestDataSource
	tmpDataSource := &v1beta1.IvoryClusterDataSource{
...
func (r *Reconciler) createRestoreConfig(ctx context.Context, ivoryCluster *v1beta1.IvoryCluster,
	configHash string) error {

	ivoryclusterWithMockedBackups := ivoryCluster.DeepCopy()
	ivoryclusterWithMockedBackups.Spec.Backups.PGBackRest.Global = ivoryCluster.Spec.
		DataSource.PGBackRest.Global
	ivoryclusterWithMockedBackups.Spec.Backups.PGBackRest.Repos = []v1beta1.PGBackRestRepo{
		ivoryCluster.Spec.DataSource.PGBackRest.Repo,
	}

	return r.reconcilePGBackRestConfig(ctx, ivoryclusterWithMockedBackups,
		"", configHash, "", "", []string{})
}

// copyRestoreConfiguration copies pgBackRest configuration from another cluster for use by
// the current IvoryCluster (e.g. when restoring across namespaces, and the configuration
// for the source cluster needs to be copied into the IvoryCluster's local namespace).
func (r *Reconciler) copyRestoreConfiguration(ctx context.Context,
	cluster, sourceCluster *v1beta1.IvoryCluster,
) error {
	var err error
...
func (r *Reconciler) copyConfigurationResources(ctx context.Context, cluster,
	sourceCluster *v1beta1.IvoryCluster) error {
	...
	for i := range sourceCluster.Spec.Backups.PGBackRest.Configuration {
		...
		if sourceCluster.Spec.Backups.PGBackRest.Configuration[i].Secret != nil {
			...
			// update the copy of the source IvoryCluster to add the new Secret
		...
		if sourceCluster.Spec.Backups.PGBackRest.Configuration[i].ConfigMap != nil {
			...
			// update the copy of the source IvoryCluster to add the new ConfigMap
			// projection(s) to the restore Job
...
func (r *Reconciler) reconcilePGBackRestConfig(ctx context.Context,
	...
	backrestConfig := pgbackrest.CreatePGBackRestConfigMapIntent(ivoryCluster, repoHostName,
		configHash, serviceName, serviceNamespace, instanceNames)
	if err := controllerutil.SetControllerReference(ivoryCluster, backrestConfig,
		r.Client.Scheme()); err != nil {
		return err
	}
	...
	repoHostConfigured := pgbackrest.DedicatedRepoHostEnabled(ivoryCluster)
...
func (r *Reconciler) reconcilePGBackRestSecret(ctx context.Context,
	cluster *v1beta1.IvoryCluster, repoHost *appsv1.StatefulSet,
	rootCA *pki.RootCertificateAuthority) error {
...
func (r *Reconciler) reconcilePGBackRestRBAC(ctx context.Context,
	ivoryCluster *v1beta1.IvoryCluster) (*corev1.ServiceAccount, error) {

	sa := &corev1.ServiceAccount{ObjectMeta: naming.PGBackRestRBAC(ivoryCluster)}
	sa.SetGroupVersionKind(corev1.SchemeGroupVersion.WithKind("ServiceAccount"))

	role := &rbacv1.Role{ObjectMeta: naming.PGBackRestRBAC(ivoryCluster)}
	role.SetGroupVersionKind(rbacv1.SchemeGroupVersion.WithKind("Role"))

	binding := &rbacv1.RoleBinding{ObjectMeta: naming.PGBackRestRBAC(ivoryCluster)}
	binding.SetGroupVersionKind(rbacv1.SchemeGroupVersion.WithKind("RoleBinding"))

	if err := r.setControllerReference(ivoryCluster, sa); err != nil {
		return nil, errors.WithStack(err)
	}
	if err := r.setControllerReference(ivoryCluster, binding); err != nil {
		return nil, errors.WithStack(err)
	}
	if err := r.setControllerReference(ivoryCluster, role); err != nil {
		return nil, errors.WithStack(err)
	}

	sa.Annotations = naming.Merge(ivoryCluster.Spec.Metadata.GetAnnotationsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetAnnotationsOrNil())
	sa.Labels = naming.Merge(ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestLabels(ivoryCluster.GetName()))
	binding.Annotations = naming.Merge(ivoryCluster.Spec.Metadata.GetAnnotationsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetAnnotationsOrNil())
	binding.Labels = naming.Merge(ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestLabels(ivoryCluster.GetName()))
	role.Annotations = naming.Merge(ivoryCluster.Spec.Metadata.GetAnnotationsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetAnnotationsOrNil())
	role.Labels = naming.Merge(ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestLabels(ivoryCluster.GetName()))
	...
	role.Rules = pgbackrest.Permissions(ivoryCluster)
...
// StatefulSet according to a specific IvoryCluster custom resource.
func (r *Reconciler) reconcileDedicatedRepoHost(ctx context.Context,
	...
	defer func() {
		repoHostReady := metav1.Condition{
			ObservedGeneration: ivoryCluster.GetGeneration(),
			Type:               ConditionRepoHostReady,
		}
		if ivoryCluster.Status.PGBackRest.RepoHost == nil {
		...
		} else if ivoryCluster.Status.PGBackRest.RepoHost.Ready {
		...
		meta.SetStatusCondition(&ivoryCluster.Status.Conditions, repoHostReady)
	}()
	var isCreate bool
	if len(repoResources.hosts) == 0 {
		name := fmt.Sprintf("%s-%s", ivoryCluster.GetName(), "repo-host")
	...
	repoHost, err := r.applyRepoHostIntent(ctx, ivoryCluster, repoHostName, repoResources,
		observedInstances)
	if err != nil {
	...
	ivoryCluster.Status.PGBackRest.RepoHost = getRepoHostStatus(repoHost)
...
func (r *Reconciler) reconcileManualBackup(ctx context.Context,
	ivoryCluster *v1beta1.IvoryCluster, manualBackupJobs []*batchv1.Job,
	serviceAccount *corev1.ServiceAccount, instances *observedInstances) error {

	manualAnnotation := ivoryCluster.GetAnnotations()[naming.PGBackRestBackup]
	manualStatus := ivoryCluster.Status.PGBackRest.ManualBackup
	...
	if len(manualBackupJobs) > 0 {
		...
		if manualStatus != nil && manualStatus.ID == backupID {
			if completed {
				meta.SetStatusCondition(&ivoryCluster.Status.Conditions, metav1.Condition{
					ObservedGeneration: ivoryCluster.GetGeneration(),
	...
	// pgBackRest connects to a IvorySQL instance that is not in recovery to
	...
	// nothing to reconcile if there is no ivory or if a manual backup has not been
	// requested
	//
	...
	if !clusterWritable || manualAnnotation == "" ||
		ivoryCluster.Spec.Backups.PGBackRest.Manual == nil {
		return nil
	}
	...
	if manualStatus == nil || manualStatus.ID != manualAnnotation {
		...
		meta.RemoveStatusCondition(&ivoryCluster.Status.Conditions,
			ConditionManualBackupSuccessful)

		ivoryCluster.Status.PGBackRest.ManualBackup = manualStatus
	}
	...
	if pgbackrest.DedicatedRepoHostEnabled(ivoryCluster) {
		condition := meta.FindStatusCondition(ivoryCluster.Status.Conditions, ConditionRepoHostReady)
		if condition == nil || condition.Status != metav1.ConditionTrue {
			return nil
		}
	}
	...
	condition := meta.FindStatusCondition(ivoryCluster.Status.Conditions,
	...
	repoName := ivoryCluster.Spec.Backups.PGBackRest.Manual.RepoName
	for _, repo := range ivoryCluster.Status.PGBackRest.Repos {
	...
	if !statusFound {
		r.Recorder.Eventf(ivoryCluster, corev1.EventTypeWarning, "InvalidBackupRepo",
			"Unable to find status for %q as configured for a manual backup.  Please ensure "+
				"this repo is defined in the spec.", repoName)
		return nil
	}
	if !stanzaCreated {
		r.Recorder.Eventf(ivoryCluster, corev1.EventTypeWarning, "StanzaNotCreated",
			"Stanza not created for %q as specified for a manual backup", repoName)
		return nil
	}

	var repo v1beta1.PGBackRestRepo
	for i := range ivoryCluster.Spec.Backups.PGBackRest.Repos {
		if ivoryCluster.Spec.Backups.PGBackRest.Repos[i].Name == repoName {
			repo = ivoryCluster.Spec.Backups.PGBackRest.Repos[i]
		}
	}
	if repo.Name == "" {
	...
	// Since '--repo' can be set with or without an equals ('=') sign, we check for both
	// usage patterns.
	backupOpts := ivoryCluster.Spec.Backups.PGBackRest.Manual.Options
	for _, opt := range backupOpts {
		if strings.Contains(opt, "--repo=") || strings.Contains(opt, "--repo ") {
			r.Recorder.Eventf(ivoryCluster, corev1.EventTypeWarning, "InvalidManualBackup",
	...
	labels = naming.Merge(ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestBackupJobLabels(ivoryCluster.GetName(), repoName,
			naming.BackupManual))
	annotations = naming.Merge(ivoryCluster.Spec.Metadata.GetAnnotationsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetAnnotationsOrNil(),
		map[string]string{
	...
	if err := controllerutil.SetControllerReference(ivoryCluster, backupJob,
		r.Client.Scheme()); err != nil {
		return errors.WithStack(err)
	}
...
func (r *Reconciler) reconcileReplicaCreateBackup(ctx context.Context,
	ivoryCluster *v1beta1.IvoryCluster, instances *observedInstances,
	replicaCreateBackupJobs []*batchv1.Job,
	...
	for i, repo := range ivoryCluster.Status.PGBackRest.Repos {
		if repo.Name == replicaCreateRepo.Name {
			replicaCreateRepoStatus = &ivoryCluster.Status.PGBackRest.Repos[i]
			break
		}
	}
	...
	defer func() {
		...
		meta.SetStatusCondition(&ivoryCluster.Status.Conditions, replicaCreate)
	}()
	...
	// return early when there is no ivory, no repo, or the backup is already complete.
	//
	// TODO (andrewlecuyer): Since reconciliation doesn't currently occur when a leader is elected,
	// the operator may not get another chance to create the backup if a writable instance is not
	// detected, and it then returns without requeing.  To ensure this doesn't occur and that the
	...
	condition := meta.FindStatusCondition(ivoryCluster.Status.Conditions, ConditionReplicaRepoReady)
	if condition != nil {
	...
	_, containerName, err := getPGBackRestExecSelector(ivoryCluster, replicaCreateRepo)
	if err != nil {
		return errors.WithStack(err)
	}
	...
	// IvoryCluster status as required
	var job *batchv1.Job
	...
	labels = naming.Merge(ivoryCluster.Spec.Metadata.GetLabelsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetLabelsOrNil(),
		naming.PGBackRestBackupJobLabels(ivoryCluster.GetName(),
			ivoryCluster.Spec.Backups.PGBackRest.Repos[0].Name, naming.BackupReplicaCreate))
	annotations = naming.Merge(ivoryCluster.Spec.Metadata.GetAnnotationsOrNil(),
		ivoryCluster.Spec.Backups.PGBackRest.Metadata.GetAnnotationsOrNil(),
		map[string]string{
...
func (r *Reconciler) reconcileRepos(ctx context.Context,
	ivoryCluster *v1beta1.IvoryCluster, extConfigHashes map[string]string,
	...
	for i, repo := range ivoryCluster.Spec.Backups.PGBackRest.Repos {
		// the repo at index 0 is the replica creation repo
		if i == 0 {
			replicaCreateRepo = ivoryCluster.Spec.Backups.PGBackRest.Repos[i]
		}
	...
	ivoryCluster.Status.PGBackRest.Repos =
		getRepoVolumeStatus(ivoryCluster.Status.PGBackRest.Repos, repoVols, extConfigHashes,
...
// pgBackRest repositories configured for a IvoryCluster.  If the bool returned from this
...
func (r *Reconciler) reconcileStanzaCreate(ctx context.Context,
	...
	defer func() {
		var replicaCreateRepoStatus *v1beta1.RepoStatus
		if len(ivoryCluster.Spec.Backups.PGBackRest.Repos) == 0 {
			return
		}
		replicaCreateRepoName := ivoryCluster.Spec.Backups.PGBackRest.Repos[0].Name
		for i, repo := range ivoryCluster.Status.PGBackRest.Repos {
		...
		meta.SetStatusCondition(&ivoryCluster.Status.Conditions, replicaCreateRepoReady)
	}()

	// determine if the cluster has been initialized. pgBackRest compares the
	// local IvorySQL data directory to information it sees in a IvorySQL
	...
	stanzasCreated := true
	for _, repoStatus := range ivoryCluster.Status.PGBackRest.Repos {
	...
	if err != nil {
		...
		r.Recorder.Event(ivoryCluster, corev1.EventTypeWarning, EventUnableToCreateStanzas,
			err.Error())
	...
	r.Recorder.Event(ivoryCluster, corev1.EventTypeNormal, EventStanzasCreated,
		"pgBackRest stanza creation completed successfully")

	// if no errors then stanza(s) created successfully
	for i := range ivoryCluster.Status.PGBackRest.Repos {
		ivoryCluster.Status.PGBackRest.Repos[i].StanzaCreated = true
	}
...
func getPGBackRestExecSelector(ivoryCluster *v1beta1.IvoryCluster,
	...
	if repo.Volume != nil {
		podSelector = naming.PGBackRestDedicatedSelector(ivoryCluster.GetName())
		containerName = naming.PGBackRestRepoContainerName
	} else {
		podSelector, err = naming.AsSelector(naming.ClusterPrimary(ivoryCluster.GetName()))
...
func (r *Reconciler) reconcileScheduledBackups(
	ctx context.Context, cluster *v1beta1.IvoryCluster, sa *corev1.ServiceAccount,
	cronjobs []*batchv1.CronJob,
) bool {
...
func (r *Reconciler) reconcilePGBackRestCronJob(
	ctx context.Context, cluster *v1beta1.IvoryCluster, repo v1beta1.PGBackRestRepo,
