...
- labelSelector:
    ...
    - key: ivory-operator.highgo.com/data
      ...
      - pgbackrest
    matchLabels:
      ivory-operator.highgo.com/cluster: hippocluster
  maxSkew: 1
...
func TestGetPGBackRestExecSelector(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedSelector: "ivory-operator.highgo.com/cluster=hippo," +
			"ivory-operator.highgo.com/pgbackrest=," +
			"ivory-operator.highgo.com/pgbackrest-dedicated=",
...
func TestReconcileReplicaCreateBackup(t *testing.T) {
	...
	for _, env := range container.Env {
		...
		case "SELECTOR":
			assert.Assert(t, env.Value == "ivory-operator.highgo.com/cluster=hippocluster,"+
				"ivory-operator.highgo.com/pgbackrest=,"+
				"ivory-operator.highgo.com/pgbackrest-dedicated=")
		}
	}
...
- command:
  ...
  - name: NAMESPACE
  - name: SELECTOR
    value: ivory-operator.highgo.com/cluster=,ivory-operator.highgo.com/instance,ivory-operator.highgo.com/role=master
  name: pgbackrest
  resources: {}
  securityContext:
...
func TestSetScheduledJobStatus(t *testing.T) {
	...
	t.Run("set scheduled backup status", func(t *testing.T) {
		...
		testJob := &batchv1.Job{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:   "TestJob",
				Labels: map[string]string{"ivory-operator.highgo.com/pgbackrest-cronjob": "full"},
			},
