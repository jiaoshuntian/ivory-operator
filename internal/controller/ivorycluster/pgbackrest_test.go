...
func fakeIvoryCluster(clusterName, namespace, clusterUID string,
	includeDedicatedRepo bool) *v1beta1.IvoryCluster {
		...
		Spec: v1beta1.IvoryClusterSpec{
			...
			Backups: v1beta1.Backups{
				PGBackRest: v1beta1.PGBackRestArchive{
					Image: "example.com/highgo-pgbackrest:test",
	...
	if includeDedicatedRepo {
		ivoryCluster.Spec.Backups.PGBackRest.Repos[0] = v1beta1.PGBackRestRepo{
			Name: "repo1",
		...
		ivoryCluster.Spec.Backups.PGBackRest.RepoHost = &v1beta1.PGBackRestRepoHost{
			PriorityClassName: initialize.String("some-priority-class"),
			Resources:         corev1.ResourceRequirements{},
	...
	ivoryCluster.Spec.Backups.PGBackRest.Repos[0].BackupSchedules = &v1beta1.PGBackRestBackupSchedules{
...
func TestReconcilePGBackRest(t *testing.T) {
	...
	ivoryCluster := fakeIvoryCluster(clusterName, ns.GetName(), clusterUID, true)

	// create a service account to test with
	serviceAccount, err := r.reconcilePGBackRestRBAC(ctx, ivoryCluster)
	...
	// set status
	ivoryCluster.Status = v1beta1.IvoryClusterStatus{
	...
	result, err := r.reconcilePGBackRest(ctx, ivoryCluster, instances, rootCA)
	if err != nil || result != (reconcile.Result{}) {
		t.Errorf("unable to reconcile pgBackRest: %v", err)
	}

	// repo is the first defined repo
	repo := ivoryCluster.Spec.Backups.PGBackRest.Repos[0]
	...
	t.Run("verify pgbackrest dedicated repo StatefulSet", func(t *testing.T) {
		...
		for _, r := range repo.GetOwnerReferences() {
			if r.Kind == "IvoryCluster" && r.Name == clusterName &&
...
- labelSelector:
    ...
    - key: ivory-operator.ivorysql.org/data
      ...
      - pgbackrest
    matchLabels:
      ivory-operator.ivorysql.org/cluster: hippocluster
  maxSkew: 1
...
- labelSelector:
  ...
  whenUnsatisfiable: ScheduleAnyway
		...
		repoHostStatus := ivoryCluster.Status.PGBackRest.RepoHost
		...
		for _, c := range ivoryCluster.Status.Conditions {
		...
		if err := wait.Poll(time.Second/2, Scale(time.Second*2), func() (bool, error) {
			if err := tClient.List(ctx, events, &client.MatchingFields{
				"involvedObject.kind":      "IvoryCluster",
	...
	t.Run("verify pgbackrest repo volumes", func(t *testing.T) {
		...
		for _, r := range ivoryCluster.Spec.Backups.PGBackRest.Repos {
			if r.Volume == nil {
				continue
			}
	...
	t.Run("verify pgbackrest configuration", func(t *testing.T) {
		...
		if err := tClient.Get(ctx, types.NamespacedName{
			Name:      naming.PGBackRestConfig(ivoryCluster).Name,
	...
	t.Run("verify pgbackrest schedule cronjob", func(t *testing.T) {
		...
		requeue := r.reconcileScheduledBackups(ctx, ivoryCluster, serviceAccount, fakeObservedCronJobs())
		assert.Assert(t, !requeue)

		returnedCronJob := &batchv1.CronJob{}
		if err := tClient.Get(ctx, types.NamespacedName{
			Name:      ivoryCluster.Name + "-repo1-full",
			Namespace: ivoryCluster.GetNamespace(),
	...
	t.Run("pgbackrest schedule suspended status", func(t *testing.T) {
		...
		t.Run("shutdown", func(t *testing.T) {
			*ivoryCluster.Spec.Shutdown = true
			ivoryCluster.Spec.Standby = nil

			requeue := r.reconcileScheduledBackups(ctx,
				ivoryCluster, serviceAccount, fakeObservedCronJobs())
			assert.Assert(t, !requeue)
		...
		t.Run("standby", func(t *testing.T) {
			*ivoryCluster.Spec.Shutdown = false
			ivoryCluster.Spec.Standby = &v1beta1.IvoryStandbySpec{
				Enabled: true,
			}
...
func TestReconcilePGBackRestRBAC(t *testing.T) {
	...
	err = tClient.Get(ctx, types.NamespacedName{
		Name:      naming.PGBackRestRBAC(ivoryCluster).Name,
		Namespace: ivoryCluster.GetNamespace(),
	}, sa)
	assert.NilError(t, err)
...
func TestReconcileStanzaCreate(t *testing.T) {
	...
	instances := newObservedInstances(ivoryCluster, nil, []corev1.Pod{{
		ObjectMeta: metav1.ObjectMeta{
			Annotations: map[string]string{"status": `"role":"master"`},
			Labels: map[string]string{
				naming.LabelCluster:  ivoryCluster.GetName(),
	...
	configHashMistmatch, err := r.reconcileStanzaCreate(ctx, ivoryCluster, instances, "abcde12345")
	assert.NilError(t, err)
	...
	for _, r := range ivoryCluster.Status.PGBackRest.Repos {
	...
	ivoryCluster.Status.Patroni = v1beta1.PatroniStatus{
		SystemIdentifier: "6952526174828511264",
	}

	configHashMismatch, err := r.reconcileStanzaCreate(ctx, ivoryCluster, instances, "abcde12345")
...
func TestGetPGBackRestExecSelector(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedSelector: "ivory-operator.ivorysql.org/cluster=hippo," +
...
func TestReconcileReplicaCreateBackup(t *testing.T) {
	...
	replicaCreateRepo := ivoryCluster.Spec.Backups.PGBackRest.Repos[0]
	...
	err := r.reconcileReplicaCreateBackup(ctx, ivoryCluster, instances,
	...
	err = tClient.List(ctx, jobs, &client.ListOptions{
		Namespace: ivoryCluster.Namespace,
	...
	for _, env := range container.Env {
		...
		case "COMMAND_OPTS":
			assert.Assert(t, env.Value == "--stanza=db --repo=1 --pg-version-force=13")
		case "COMPARE_HASH":
		...
		case "SELECTOR":
			assert.Assert(t, env.Value == "ivory-operator.ivorysql.org/cluster=hippocluster,"+
				"ivory-operator.ivorysql.org/pgbackrest=,"+
				"ivory-operator.ivorysql.org/pgbackrest-dedicated=")
		}
	}
	...
	condition := meta.FindStatusCondition(ivoryCluster.Status.Conditions, ConditionReplicaCreate)
...
func TestReconcileManualBackup(t *testing.T) {
	...
	testCases := []struct {
		...
		// conditions to apply to the mock ivorycluster
		clusterConditions map[string]metav1.ConditionStatus
		// the status to apply to the mock ivorycluster
		status *v1beta1.IvoryClusterStatus
		// the ID used to populate the "backup" annotation for the test (can be empty)
		backupId string
		// the manual backup field to define in the ivorycluster spec for the test
	...
	for _, dedicated := range []bool{true, false} {
		for i, tc := range testCases {
			...
			t.Run(tc.testDesc, func(t *testing.T) {
				...
				ivoryCluster := fakeIvoryCluster(clusterName, ns.GetName(), "", dedicated)
				ivoryCluster.Spec.Backups.PGBackRest.Manual = tc.manual
				ivoryCluster.Annotations = map[string]string{naming.PGBackRestBackup: tc.backupId}
				assert.NilError(t, tClient.Create(ctx, ivoryCluster))

				ivoryCluster.Status = *tc.status
				for condition, status := range tc.clusterConditions {
					meta.SetStatusCondition(&ivoryCluster.Status.Conditions, metav1.Condition{
						Type: condition, Reason: "testing", Status: status})
				}
				assert.NilError(t, tClient.Status().Update(ctx, ivoryCluster))

				currentJobs := []*batchv1.Job{}
				if tc.createCurrentJob {
					job := fakeJob(ivoryCluster.GetName(), tc.manual.RepoName)
				...
				err := r.reconcileManualBackup(ctx, ivoryCluster, currentJobs, sa, instances)
				...
				if tc.expectReconcile {
					...
					assert.Assert(t, ivoryCluster.Status.PGBackRest.ManualBackup != nil)
					assert.Assert(t, ivoryCluster.Status.PGBackRest.ManualBackup.ID != "")
					...
					if tc.expectedEventReason != "" {
						...
						err = wait.Poll(time.Second/2, Scale(time.Second*2), func() (bool, error) {
							if err := tClient.List(ctx, events, &client.MatchingFields{
								...
								"involvedObject.uid":       string(ivoryCluster.GetUID()),
...
func TestReconcileIvoryClusterDataSource(t *testing.T) {
	tEnv, tClient := setupKubernetes(t)
	...
	for _, dedicated := range []bool{true, false} {
		testCases := []struct {
			...
			dataSource: &v1beta1.DataSource{IvoryCluster: &v1beta1.IvoryClusterDataSource{
		...
		for i, tc := range testCases {
			...
			t.Run(tc.desc, func(t *testing.T) {
				...
				sourceCluster := fakeIvoryCluster(tc.sourceClusterName, namespace,
				...
				var pgclusterDataSource *v1beta1.IvoryClusterDataSource
				if tc.dataSource != nil {
					pgclusterDataSource = tc.dataSource.IvoryCluster
				}
				err := r.reconcileIvoryClusterDataSource(ctx, cluster, pgclusterDataSource,
					"testhash", nil, rootCA)
				assert.NilError(t, err)
...
func TestCopyConfigurationResources(t *testing.T) {
	...
	sourceCluster := func(testNum string) *v1beta1.IvoryCluster {
		return &v1beta1.IvoryCluster{
			ObjectMeta: metav1.ObjectMeta{
	...
	cluster := func(testNum, scName, scNamespace string) *v1beta1.IvoryCluster {
		return &v1beta1.IvoryCluster{
			ObjectMeta: metav1.ObjectMeta{
...
func TestGenerateBackupJobIntent(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		spec, err := generateBackupJobSpecIntent(
			&v1beta1.IvoryCluster{}, v1beta1.PGBackRestRepo{},
			"",
			nil, nil,
		)
		assert.NilError(t, err)
...
- command:
  ...
  - name: COMMAND
    value: backup
  - name: COMMAND_OPTS
    value: --stanza=db --repo= --pg-version-force=0
  - name: COMPARE_HASH
    value: "true"
  - name: CONTAINER
    value: database
  - name: NAMESPACE
  - name: SELECTOR
    value: ivory-operator.ivorysql.org/cluster=,ivory-operator.ivorysql.org/instance,ivory-operator.ivorysql.org/role=master
  name: pgbackrest
  resources: {}
  securityContext:
...
func TestGenerateRepoHostIntent(t *testing.T) {
	...
	t.Run("empty", func(t *testing.T) {
		_, err := r.generateRepoHostIntent(&v1beta1.IvoryCluster{}, "", &RepoResources{},
			&observedInstances{})
		assert.NilError(t, err)
	})
	...
	t.Run("IVY instances observed, do not shutdown repo host", func(t *testing.T) {
		cluster := &v1beta1.IvoryCluster{
			Spec: v1beta1.IvoryClusterSpec{
	...
	t.Run("No IVY instances observed, shutdown repo host", func(t *testing.T) {
		cluster := &v1beta1.IvoryCluster{
			Spec: v1beta1.IvoryClusterSpec{
				Shutdown: initialize.Bool(true),
			},
...
func TestGenerateRestoreJobIntent(t *testing.T) {
	...
	t.Run("empty", func(t *testing.T) {
		err := r.generateRestoreJobIntent(&v1beta1.IvoryCluster{}, "", "",
			[]string{}, []corev1.VolumeMount{}, []corev1.Volume{},
			&v1beta1.IvoryClusterDataSource{}, &batchv1.Job{})
		assert.NilError(t, err)
	})
	...
	dataSource := &v1beta1.IvoryClusterDataSource{
		...
		// TestReconcileIvoryClusterDataSource
		Resources: corev1.ResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceStorage: resource.MustParse("1Gi"),
			},
...
func TestPrepareForRestore(t *testing.T) {
	...
	for _, dedicated := range []bool{true, false} {
		testCases := []struct {
			desc            string
			createResources func(t *testing.T, cluster *v1beta1.IvoryCluster) (*batchv1.Job, []corev1.Endpoints)
			...
			fakeObserved: &observedInstances{forCluster: []*Instance{{
				...
				Spec: &v1beta1.IvoryInstanceSetSpec{Name: primaryInstanceSetName},
				Pods: []*corev1.Pod{{
					ObjectMeta: metav1.ObjectMeta{
						Labels: map[string]string{naming.LabelRole: naming.RolePatroniLeader},
					},
		...
		for i, tc := range testCases {
			...
			t.Run(tc.testDesc, func(t *testing.T) {
				...
				if tc.cronJobs {
					...
					requeue = r.reconcileScheduledBackups(ctx, ivoryCluster, sa, existingCronJobs)
				} else {
					requeue = r.reconcileScheduledBackups(ctx, ivoryCluster, sa, fakeObservedCronJobs())
				}
				if !tc.expectReconcile && !tc.expectRequeue {
					...
					for _, backupType := range backupTypes {
						...
						if tc.cronJobs {
							...
							cronJobName = ivoryCluster.Name + "-repo1-" + backupType
						}
...
func TestSetScheduledJobStatus(t *testing.T) {
	...
	t.Run("set scheduled backup status", func(t *testing.T) {
		...
		testJob := &batchv1.Job{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:   "TestJob",
				Labels: map[string]string{"ivory-operator.ivorysql.org/pgbackrest-cronjob": "full"},
			},
		...
		assert.Assert(t, len(ivoryCluster.Status.PGBackRest.ScheduledBackups) > 0)
		assert.Equal(t, ivoryCluster.Status.PGBackRest.ScheduledBackups[0].Active, int32(1))
		assert.Equal(t, ivoryCluster.Status.PGBackRest.ScheduledBackups[0].Succeeded, int32(2))
		assert.Equal(t, ivoryCluster.Status.PGBackRest.ScheduledBackups[0].Failed, int32(3))
	})
