...
func (r *Reconciler) reconcilePGMonitor(ctx context.Context,
	cluster *v1beta1.IvoryCluster, instances *observedInstances,
...
// - PodExec to get setup.sql file for the ivory version
...
func (r *Reconciler) reconcilePGMonitorExporter(ctx context.Context,
	...
	// Find the IvorySQL instance that can execute SQL that writes to every
	...
	// the `EnableExporterInIvorySQL` funcs; that way we are always running
	...
	// IvorySQL is available for writes. Prepare to either add or remove
	// pgMonitor objects.

	action := func(ctx context.Context, exec ivory.Executor) error {
		return pgmonitor.EnableExporterInIvorySQL(ctx, exec, monitoringSecret, exporterDB, setup)
	}

	if !pgmonitor.ExporterEnabled(cluster) {
		action = func(ctx context.Context, exec ivory.Executor) error {
			return pgmonitor.DisableExporterInIvorySQL(ctx, exec)
		}
	}
...
func (r *Reconciler) reconcileMonitoringSecret(
	ctx context.Context,
	cluster *v1beta1.IvoryCluster) (*corev1.Secret, error) {
...
func getExporterCertSecret(cluster *v1beta1.IvoryCluster) *corev1.SecretProjection {
	if cluster.Spec.Monitoring.PGMonitor.Exporter.CustomTLSSecret != nil {
...
func configureExporterTLS(cluster *v1beta1.IvoryCluster, template *corev1.PodTemplateSpec, exporterWebConfig *corev1.ConfigMap) {
	var found bool
...
func (r *Reconciler) reconcileExporterWebConfig(ctx context.Context,
	cluster *v1beta1.IvoryCluster) (*corev1.ConfigMap, error) {
