...
const (
	// https://www.ivorysql.org/docs/current/ssl-tcp.html
...
func (r *Reconciler) reconcileRootCertificate(
	...
	*pki.RootCertificateAuthority, error,
	...
	// updated to include the ivorycluster as an owner.
	// However, unlike the leaf certificate, the ivorycluster will not be
...
func (r *Reconciler) reconcileClusterCertificate(
	...
	cluster *v1beta1.IvoryCluster, primaryService *corev1.Service,
) (
	*corev1.SecretProjection, error,
	...
	existing := &corev1.Secret{ObjectMeta: naming.IvoryTLSSecret(cluster)}
	err := errors.WithStack(client.IgnoreNotFound(
		r.Client.Get(ctx, client.ObjectKeyFromObject(existing), existing)))
	...
	intent.Labels = naming.Merge(
		...
		map[string]string{
			...
			naming.LabelClusterCertificate: "ivory-tls",
		})
	...
	// TODO(tjmoore4): The generated ivorycluster secret is only created
...
// clusterCertSecretProjection returns a secret projection of the ivorycluster's
