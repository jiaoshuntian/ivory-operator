...
const (
	...
	// ivoryNSSWrapperPrefix sets the required variables when running the NSS
	// wrapper script for the 'ivory' user
	ivoryNSSWrapperPrefix = `export NSS_WRAPPER_SUBDIR=ivory HIGHGO_NSS_USERNAME=ivory ` +
		`HIGHGO_NSS_USER_DESC="ivory" `
	// pgAdminNSSWrapperPrefix sets the required variables when running the NSS
	// wrapper script for the 'pgadmin' user
	pgAdminNSSWrapperPrefix = `export NSS_WRAPPER_SUBDIR=pgadmin HIGHGO_NSS_USERNAME=pgadmin ` +
		`HIGHGO_NSS_USER_DESC="pgadmin" `
...
if [[ ! $(cat "${NSS_WRAPPER_GROUP}") =~ ${HIGHGO_NSS_USERNAME}:x:${USER_ID} ]]; then
    echo "nss_wrapper: adding group"
    group_tmp="${NSS_WRAPPER_DIR}/group_tmp"
    cp "${NSS_WRAPPER_GROUP}" "${group_tmp}"
    sed -i "/${HIGHGO_NSS_USERNAME}:x:/d" "${group_tmp}"
    printf '${HIGHGO_NSS_USERNAME}:x:${USER_ID}:${HIGHGO_NSS_USERNAME}\n' >> "${group_tmp}"
...
// Ivory to allocate shared memory segments. This is a special directory
...
func addNSSWrapper(image string, imagePullPolicy corev1.PullPolicy, template *corev1.PodTemplateSpec) {
	...
	nssWrapperCmd := ivoryNSSWrapperPrefix + nssWrapperScript
