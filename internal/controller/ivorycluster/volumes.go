...
// repo volumes to be used by the IvoryCluster. In the case of existing
// pgData volumes, an appropriate instance set name is defined that will be
// used for the IvoryCluster. Existing pg_wal volumes MUST be defined along
...
// the IvoryCluster spec. A boolean value is return to indicate whether
// the main control loop should return early.
func (r *Reconciler) reconcileDirMoveJobs(ctx context.Context,
	cluster *v1beta1.IvoryCluster) (bool, error) {
...
// in the given volume to the expected location before the IvoryCluster is
...
func (r *Reconciler) reconcileMovePGDataDir(ctx context.Context,
	cluster *v1beta1.IvoryCluster, moveJobs *batchv1.JobList) (bool, error) {
	...
	script := fmt.Sprintf(`echo "Preparing cluster %s volumes for IVO v5.x"
    echo "pgdata_pvc=%s"
    echo "Current IVY data directory volume contents:" 
    ls -lh "/pgdata"
    echo "Now updating IVY data directory..."
    [ -d "/pgdata/%s" ] && mv "/pgdata/%s" "/pgdata/pg%s_bootstrap"
    rm -f "/pgdata/pg%s/patroni.dynamic.json"
    echo "Updated IVY data directory contents:" 
    ls -lh "/pgdata"
    echo "IVY Data directory preparation complete"
    `, cluster.Name,
	...
	container := corev1.Container{
		...
		VolumeMounts:    []corev1.VolumeMount{ivory.DataVolumeMount()},
	}
	if len(cluster.Spec.InstanceSets) > 0 {
	...
	jobSpec := &batchv1.JobSpec{
		Template: corev1.PodTemplateSpec{
			...
			Spec: corev1.PodSpec{
				...
				SecurityContext:  ivory.PodSecurityContext(cluster),
...
func (r *Reconciler) reconcileMoveWALDir(ctx context.Context,
	...
	script := fmt.Sprintf(`echo "Preparing cluster %s volumes for IVO v5.x"
    echo "pg_wal_pvc=%s"
    echo "Current IVY WAL directory volume contents:"
    ls -lh "/pgwal"
    echo "Now updating IVY WAL directory..."
    [ -d "/pgwal/%s" ] && mv "/pgwal/%s" "/pgwal/%s-wal"
    echo "Updated IVY WAL directory contents:"
    ls -lh "/pgwal"
    echo "IVY WAL directory preparation complete"
    `, cluster.Name,
	...
	container := corev1.Container{
		...
		SecurityContext: initialize.RestrictedSecurityContext(),
		VolumeMounts:    []corev1.VolumeMount{ivory.WALVolumeMount()},
	}
	if len(cluster.Spec.InstanceSets) > 0 {
		container.Resources = cluster.Spec.InstanceSets[0].Resources
	}
...
// IvoryCluster is bootstrapped. It returns any errors and a boolean
...
func (r *Reconciler) reconcileMoveRepoDir(ctx context.Context,
	...
	script := fmt.Sprintf(`echo "Preparing cluster %s pgBackRest repo volume for IVO v5.x"
    echo "repo_pvc=%s"
...
func (r *Reconciler) handlePersistentVolumeClaimError(
	cluster *v1beta1.IvoryCluster, err error,
) error {
