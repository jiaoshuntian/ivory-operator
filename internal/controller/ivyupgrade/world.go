...
func (r *IvyUpgradeReconciler) observeWorld(
	ctx context.Context, upgrade *v1beta1.IvyUpgrade,
) (*World, error) {
	...
	cluster := v1beta1.NewIvoryCluster()
	err := errors.WithStack(
		r.Get(ctx, client.ObjectKey{
			Namespace: upgrade.Namespace,
			Name:      upgrade.Spec.IvoryClusterName,
		}, cluster))
...
func (w *World) populateCluster(cluster *v1beta1.IvoryCluster, err error) error {
	if err == nil {
		w.Cluster = cluster
