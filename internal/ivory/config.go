...
// - https://www.ivorysql.org/docs/current/runtime-config-file-locations.html
...
// Environment returns the environment variables required to invoke IvorySQL
// utilities.
func Environment(cluster *v1beta1.IvoryCluster) []corev1.EnvVar {
	return []corev1.EnvVar{
		// - https://www.ivorysql.org/docs/current/reference-server.html
		{
			Name:  "PGDATA",
			Value: ConfigDirectory(cluster),
		},

		// - https://www.ivorysql.org/docs/current/libpq-envars.html
		{
			Name:  "PGHOST",
...
// reloadCommand returns an entrypoint that convinces IvorySQL to reload
...
func reloadCommand(name string) []string {
	...
	// signal IvorySQL, and print the observed timestamp.
	//
	// IvorySQL v10 reads its server certificate files during reload (SIGHUP).
	// - https://www.ivorysql.org/docs/current/ssl-tcp.html#SSL-SERVER-FILES
	// - https://www.ivorysql.org/docs/current/app-ivory.html
	//
	// IvorySQL reads its replication credentials every time it opens a
	...
	// but IvorySQL requires client keys to not be readable by other users.
	// - https://www.ivorysql.org/docs/current/libpq-ssl.html
...
func startupCommand(
	...
	script := strings.Join([]string{
		...
		`results() { printf '::ivory-operator: %s::%s\n' "$@"; }`,
