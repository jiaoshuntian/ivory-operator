...
func TestCreateDatabasesInIvorySQL(t *testing.T) {
	...
	t.Run("Arguments", func(t *testing.T) {
		...
		assert.Equal(t, expected, CreateDatabasesInIvorySQL(ctx, exec, nil))
	})
...
`, "\n"))
		...
		assert.NilError(t, CreateDatabasesInIvorySQL(ctx, exec, nil))
		assert.Equal(t, calls, 1)

		assert.NilError(t, CreateDatabasesInIvorySQL(ctx, exec, []string{}))
...
`))
		...
		assert.NilError(t, CreateDatabasesInIvorySQL(ctx, exec,
