...
func NewHBAs() HBAs {
	return HBAs{
		Mandatory: []HostBasedAuthentication{
			// The "ivorysql" superuser must always be able to connect locally.
			*NewHBA().Local().User("ivorysql").Method("peer"),

			// The replication user must always connect over TLS using certificate
			// authentication. Patroni also connects to the "ivorysql" database
