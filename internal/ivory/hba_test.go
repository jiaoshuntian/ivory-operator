...
hostssl  replication  "_highgorepl"  all   cert
hostssl  "ivorysql"   "_highgorepl"  all   cert
host     all          "_highgorepl"  all   reject
	`))
...
func TestHostBasedAuthentication(t *testing.T) {
	assert.Equal(t, `local all "ivory" peer`,
		NewHBA().Local().User("ivory").Method("peer").String())
