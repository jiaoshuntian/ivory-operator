...
// MD5Password implements the IvoryPassword interface for hashing passwords
// using the IvorySQL MD5 method
type MD5Password struct {
	// password is the plaintext password
	password string
	// username is the IvorySQL username
	username string
}

// Build creates the MD5 password format for IvorySQL which resembles
// "md5" + md5("password" + "username")
func (m *MD5Password) Build() (string, error) {
	// create the plaintext password/salt that IvorySQL expects as a byte string
	plaintext := []byte(m.password + m.username)
	// finish the transformation by getting the string value of the MD5 hash and
	// encoding it in hexadecimal for IvorySQL, appending "md5" to the front
	// #nosec G401
