...
func DataVolumeMount() corev1.VolumeMount {
	return corev1.VolumeMount{Name: "ivory-data", MountPath: dataMountPath}
}
...
func WALVolumeMount() corev1.VolumeMount {
	return corev1.VolumeMount{Name: "ivory-wal", MountPath: walMountPath}
}
...
func InstancePod(ctx context.Context,
	...
	certVolume := corev1.Volume{
		...
		VolumeSource: corev1.VolumeSource{
			Projected: &corev1.ProjectedVolumeSource{
				// IvorySQL expects client certificate keys to not be readable
				// by any other user.
	...
	container := corev1.Container{
		...
		Image:           config.IvoryContainerImage(inCluster),
	...
	startup := corev1.Container{
		Name: naming.ContainerIvoryStartup,
...
func PodSecurityContext(cluster *v1beta1.IvoryCluster) *corev1.PodSecurityContext {
	...
	// Otherwise, set a filesystem group so IvorySQL can write to files
