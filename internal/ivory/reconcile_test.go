...
- mountPath: /etc/database-containerinfo
  ...
  readOnly: true
- mountPath: /pgwal
  name: ivory-wal`), "expected WAL and downwardAPI mounts in %q container", pod.Containers[0].Name)
...
- mountPath: /pgdata
  name: ivory-data
- mountPath: /pgwal
  name: ivory-wal`), "expected WAL mount, no downwardAPI mount in %q container", pod.InitContainers[0].Name)
...
- mountPath: /pgconf/tls
  ...
  readOnly: true
- mountPath: /pgdata
  name: ivory-data`), "expected WAL mount, no downwardAPI mount in %q container", pod.InitContainers[0].Name)
	})

	t.Run("WithCustomSidecarContainer", func(t *testing.T) {
		sidecarInstance := new(v1beta1.IvoryInstanceSetSpec)
