...
func clusterYAML(
		...
		"postgresql": map[string]interface{}{
			...
			"authentication": map[string]interface{}{
				"replication": map[string]interface{}{
					...
					"username":    ivory.ReplicationUser,
				},
	...
	if !ClusterBootstrapped(cluster) {
		...
		root["bootstrap"] = map[string]interface{}{
			"dcs": DynamicConfiguration(cluster, configuration, ivyHBAs, pgParameters),
...
func DynamicConfiguration(
	...
	hba := make([]string, 0, len(ivyHBAs.Mandatory))
	for i := range ivyHBAs.Mandatory {
		hba = append(hba, ivyHBAs.Mandatory[i].String())
	}
	...
	if len(hba) == len(ivyHBAs.Mandatory) {
		for i := range ivyHBAs.Default {
			hba = append(hba, ivyHBAs.Default[i].String())
		}
	}
...
func instanceEnvironment(
	...
	variables := []corev1.EnvVar{
		...
		// PostgreSQL must be restarted when changing this value.
		{
			...
			Value: fmt.Sprintf("%s.%s:%d", "$(PATRONI_NAME)", podSubdomain, ivoryPort),
		},
		...
		// Patroni must be restarted when changing this value.
		{
			...
			Value: ivory.ConfigDirectory(cluster),
		},
		...
		// Patroni must be restarted when changing this value.
		{
			...
			Value: ivory.DataDirectory(cluster),
		},
...
func instanceYAML(
	...
	postgresql["basebackup"] = []string{
		...
		"waldir=" + ivory.WALDirectory(cluster, instance),
	}
	...
	if !ClusterBootstrapped(cluster) {
		...
		if isRestore || isDataSource {
			data_dir := ivory.DataDirectory(cluster)
			root["bootstrap"] = map[string]interface{}{
				"method": "existing",
