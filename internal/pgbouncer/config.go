...
const (
	...
	authFileProjectionPath  = "~ivory-operator/users.txt"
	emptyFileProjectionPath = "pgbouncer.ini"
	iniFileProjectionPath   = "~ivory-operator.ini"
...
func authFileContents(password string) []byte {
	...
	user1 := quote(ivorysqlUser) + " " + quote(password) + "\n"

	return []byte(user1)
}

func clusterINI(cluster *v1beta1.IvoryCluster) string {
	var (
		pgBouncerPort = *cluster.Spec.Proxy.PGBouncer.Port
		ivoryPort     = *cluster.Spec.Port
	)

	global := iniValueSet{
		// Prior to IvorySQL v12, the default setting for "extra_float_digits"
		// does not return precise float values. Applications that want
		// consistent results from different IvorySQL versions may connect
		...
		// - https://www.ivorysql.org/docs/current/runtime-config-client.html#GUC-EXTRA-FLOAT-DIGITS
		...
		// Authenticate frontend connections using passwords stored in IvorySQL.
		...
		// Require TLS encryption on connections to IvorySQL.
	...
	// to the wildcard, PgBouncer first checks for the database in IvorySQL.
	...
	databases := iniValueSet{
		"*": fmt.Sprintf("host=%s port=%d",
			naming.ClusterPrimaryService(cluster).Name, ivoryPort),
	}
