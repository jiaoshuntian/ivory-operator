...
func TestIvoryClusterWebhooks(t *testing.T) {
	var _ webhook.Defaulter = new(IvoryCluster)
}

func TestIvoryClusterDefault(t *testing.T) {
	t.Run("TypeMeta", func(t *testing.T) {
		var cluster IvoryCluster
		cluster.Default()
...
status:
  ...
  proxy:
	...
	t.Run("one instance set", func(t *testing.T) {
		var cluster IvoryCluster
		cluster.Spec.InstanceSets = []IvoryInstanceSetSpec{{}}
		cluster.Default()
...
status:
  ...
  proxy:
	...
	t.Run("empty proxy", func(t *testing.T) {
		var cluster IvoryCluster
		cluster.Spec.Proxy = new(IvoryProxySpec)
		cluster.Default()
	...
	t.Run("PgBouncer proxy", func(t *testing.T) {
		var cluster IvoryCluster
		cluster.Spec.Proxy = &IvoryProxySpec{PGBouncer: &PGBouncerPodSpec{}}
		cluster.Default()
...
func TestIvoryInstanceSetSpecDefault(t *testing.T) {
	var spec IvoryInstanceSetSpec
	spec.Default(5)
